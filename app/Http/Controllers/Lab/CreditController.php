...
class CreditController extends Controller
{
        ...
        if ($order) {
            DB::transaction(function () use ($order, $accion, $lab) {
                if ($accion === 'aprobar') {
                    ...
                    DB::table('transactions')->where('user_id', $order->maker_id)->where('amount', $order->total_fc)->where('type', 'escrow')->latest('id')->limit(1)->update(['type' => 'burn', 'description' => '🔥 Servicio consumido (Quema): ' . $order->custom_name]);
                    ...
                    DB::table('transactions')->insert(['user_id' => $order->maker_id, 'description' => "Reembolso: " . $order->custom_name, 'amount' => $order->total_fc, 'type' => 'income', 'created_at' => now()]);
                }
            });
    ...
    {
        $maker = User::where('email', trim($request->input('email_maker')))->where('role', 'maker')->first();
        if (!$maker) return redirect()->route('lab.dashboard')->with('error', "No se encontró ningún Maker registrado con ese correo.");

        $monto = floatval($request->input('monto_fc'));
        DB::table('financing_agreements')->insert([
            'lab_id' => auth()->id(), 'maker_id' => $maker->id, 'amount_initial' => $monto, 'amount_remaining' => $monto,
