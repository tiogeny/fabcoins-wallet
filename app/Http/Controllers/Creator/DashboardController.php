...
{
    /**
     * Carga el estado de cuenta y sincroniza los KPIs del Creador
     */
    public function index()
    {
        ...
        $querySaldo = DB::select("SELECT SUM(CASE WHEN type = 'income' THEN amount ELSE -amount END) as saldo FROM transactions WHERE user_id = ?", [$creador->id]);
        ...
        $misionesAbiertas = DB::table('missions as m')
            ...
            ->where('m.status', 'open')
            ->where(function($query) use ($creador) {
                $query->whereNull('m.target_creador_id')->orWhere('m.target_creador_id', $creador->id);
            })
            ->whereNotIn('m.id', function($q) use ($creador) {
                $q->select('mission_id')->from('mission_applications')->where('creador_id', $creador->id);
            })
            ->select('m.*', 'u.name as lab_name', 'u.avatar_url', 'u.slug as lab_slug')
            ->orderBy('m.created_at', 'desc')
            ->get();

        // Estado de Postulaciones Laborales del Creador
        $misPostulaciones = DB::table('mission_applications as ma')
            ->join('missions as m', 'ma.mission_id', '=', 'm.id')
            ->join('users as u', 'm.lab_id', '=', 'u.id')
            ->where('ma.creador_id', $creador->id)
            ->select('ma.*', 'm.title', 'm.reward_fc', 'm.status as mission_status', 'm.deadline', 'm.lab_id', 'm.target_creador_id', 'u.name as lab_name', 'u.slug as lab_slug')
        ...
        $misTransacciones = $creador->transacciones()->latest()->limit(50)->get();
        ...
        $historialAbonos = DB::table('transactions as t')
            ...
            ->where('m.assigned_creador_id', $creador->id)
        ...
        // Notificaciones
        $notificaciones = DB::table('notifications')->where('user_id', $creador->id)->latest()->limit(10)->get();
        ...
        $misSkillsIds = DB::table('user_skills')->where('user_id', $creador->id)->pluck('skill_id')->toArray();

        return view('creador.dashboard', compact(
            'creador', 'saldoTotal', 'misionesCompletadasKpi', 'misionesAbiertas', 'misPostulaciones',
