    ...
    <form action="{{ route('lab.mission.store') }}" method="POST">
        ...
        <div style="display: flex; flex-direction: column; gap: 15px;">
            ...
            <div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 15px;">
                ...
                <div>
                    <select name="target_maker_id" id="selector-mision-dirigida" onchange="evaluarRulesCuposMision(this)" style="width: 100%; font-weight: 600; margin-bottom: 0; background: #131722; border: 1px solid rgba(255,255,255,0.06); color: #fff; height: 40px; border-radius: 6px; font-size: 12.5px;">
            ...
            <div style="display: grid; grid-template-columns: 0.8fr 0.8fr 1fr 1.4fr; gap: 15px; align-items: center;">
                ...
                <div>
                    <label style="font-size: 10px; font-weight: 700; color: #7f8c8d; text-transform: uppercase; display: block; margin-bottom: 5px;">{{ __('messages.lbl_reward_per_maker') }}</label>
                    <input type="number" name="reward_fc" value="100" min="1" style="width: 100%; background: #131722; border: 1px solid rgba(255,255,255,0.06); color: #f1c40f; height: 38px; border-radius: 6px; font-family: 'Rajdhani', sans-serif; font-weight: 700; font-size: 15px; text-align: center;">
                </div>
                <div>
                    <label style="font-size: 10px; font-weight: 700; color: #7f8c8d; text-transform: uppercase; display: block; margin-bottom: 5px;">{{ __('messages.th_limit') }}</label>
                    <input type="date" name="deadline" value="{{ date('Y-m-d', strtotime('+7 days')) }}" style="width: 100%; background: #131722; border: 1px solid rgba(255,255,255,0.06); color: #fff; height: 38px; border-radius: 6px; font-size: 12.5px; padding: 0 10px;" required>
                </div>
                            ...
                            <td style="padding: 14px 12px;">
                                @php
                                    $postulantes = DB::table('mission_applications')
                                        ...
                                        ->select('users.name', 'users.id as maker_id', 'users.deuda_fc', 'users.deuda_lab_id', 'mission_applications.status', 'mission_applications.is_reviewed')
                                        ->get();
                                @endphp
                                                ...
                                                <div style="display: flex; gap: 6px;">
                                                    @if($p->status === 'pending')
                                                        ...
                                                        <form action="{{ route('lab.mission.assign') }}" method="POST" style="margin:0;">
                                                            ...
                                                            <input type="hidden" name="mission_id" value="{{ $m->id }}">
                                                            <input type="hidden" name="maker_id" value="{{ $p->maker_id }}">
                                                            <button type="submit" class="btn-back-minimal" style="padding: 4px 10px; font-size: 9.5px; background: rgba(52,152,219,0.05) !important; border-color: rgba(52,152,219,0.3) !important; color: #3498db !important;">{{ __('messages.btn_assign_maker') }}</button>
                                                        </form>
                                                    @elseif($p->status === 'accepted' && !$p->is_reviewed)
                                                        ...
                                                        @endphp
                                                        <button type="button" onclick="ejecutarAperturaModalAuditoria({{ $m->id }}, {{ $p->maker_id }}, '{{ $p->name }}', {{ $esDeudorDirecto ? 'true' : 'false' }}, {{ intval($m->reward_fc) }})" class="btn-back-minimal" style="padding: 5px 12px; font-size: 10px; font-weight:700; background: rgba(255,255,255,0.02) !important; border-color: {{ $colorBotonEval }} !important; color: {{ $colorBotonEval }} !important;">
                                                            {{ __($llaveBotonEval) }}
                                                        </button>
                                                    @elseif($p->status === 'accepted' && $p->is_reviewed)
...
<div id="modal-auditoria-talentos" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.75); z-index: 3000; align-items: center; justify-content: center; backdrop-filter: blur(4px);">
    <div style="background: #1c2230; width: 100%; max-width: 520px; border-radius: 14px; border: 1px solid rgba(255,255,255,0.06); box-shadow: 0 20px 50px rgba(0,0,0,0.6); overflow: hidden; padding: 24px;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 18px; border-bottom: 1px solid rgba(255,255,255,0.04); padding-bottom: 12px;">
            <h3 id="modal-titulo-auditoria" style="font-family: 'Rajdhani', sans-serif; font-weight: 700; font-size: 17px; color: #ffffff; text-transform: uppercase; margin: 0; letter-spacing: 0.5px;">{{ __('messages.lbl_evaluate_maker') }}</h3>
        ...
        <form action="{{ route('lab.mission.complete') }}" method="POST" id="form-envio-auditoria">
            ...
            <input type="hidden" name="maker_id" id="modal-field-maker-id">

            <div style="background: #131722; padding: 12px; border-radius: 8px; margin-bottom: 20px; border: 1px solid rgba(255,255,255,0.01);">
                <div style="font-size: 11px; color: #7f8c8d; text-transform: uppercase; font-weight: 700; letter-spacing: 0.5px;">{{ __('messages.lbl_assigned_maker') }}</div>
                <div id="modal-pizarra-nombre-maker" style="font-size: 15px; color: #ffffff; font-weight: 600; margin-top: 2px;">-</div>
            </div>
...
function ejecutarAperturaModalAuditoria(missionId, makerId, makerName, esDeudor, rewardFc) {
    document.getElementById('modal-field-mission-id').value = missionId;
    document.getElementById('modal-field-maker-id').value = makerId;
    document.getElementById('modal-pizarra-nombre-maker').textContent = makerName;
